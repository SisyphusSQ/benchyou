...
func MockInitFlags(cmd *cobra.Command, addr string) {
	...
	cmd.Flags().String("mysql-user", "mock", "MySQL user(Default mybenchx)")
	cmd.Flags().String("mysql-password", "", "MySQL password(Default mybenchx)")
	...
	cmd.Flags().String("ssh-user", "mybenchx", "SSH server user(Default mybenchx)")
	cmd.Flags().String("ssh-password", "mybenchx", "SSH server password(Default mybenchx)")
